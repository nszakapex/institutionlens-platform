...
4. **Observed-alignment shortlist** — max 8 assessed, non-excluded organizations; each row includes a server-built Compare inbound link (`compareHrefFor`) with accessible organization context
5. **Evidence review required** — insufficient-evidence portfolios without numeric scores (no Compare inbound in Phase 7)
6. **Attention / review queue** — max 10 deterministic attention items (not fit ranking; no Compare inbound in Phase 7)
