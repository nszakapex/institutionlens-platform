...
4. **Observed-alignment shortlist** — max 8 assessed, non-excluded organizations; each row includes server-built Compare (`compareHrefFor`) and Brief (`inboundBriefActionFor`) inbound links with accessible organization context
5. **Evidence review required** — insufficient-evidence portfolios without numeric scores (no Compare/Brief inbound)
6. **Attention / review queue** — max 10 deterministic attention items (not fit ranking; no Compare/Brief inbound)
