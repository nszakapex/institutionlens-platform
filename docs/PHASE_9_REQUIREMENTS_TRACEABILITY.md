...
**Scope:** Batches 1-5 database surface on staging through remaining read RPC; Auth binding + gateway in code; app runtime still local-demo

**Status:** Staging Batch 5 RPC applied (`20260715220000`); live verifier 13/13 with five migrations; Auth session binding + gateway transport in code; application runtime cutover (Auth fixtures + Preview live env) pending

**External infrastructure:** Staging project `qzidcqtaabubvtycstwy`; migrations through `20260715220000` applied; application tables empty; Vercel Preview still `local-demo`; no production cutover
...
| P9B2-03 | Add a server-only Supabase/Postgres adapter structure                        | Implemented    | `supabase-postgres/gateway.ts`, `adapter.ts`, `rpc-surface.ts`, `live-row-decoders.ts`; server-boundary contract                                         | SDK transport and RPC apply remain gated                          |
...
| ID       | Requirement                                                            | Classification | Evidence                                                                             | Remaining limitation                          |
| -------- | ---------------------------------------------------------------------- | -------------- | ------------------------------------------------------------------------------------ | --------------------------------------------- |
| P9B5P-01 | Server-only Supabase Auth session binding (no client tenant selection) | Prepared       | `session-context.ts`; live `session_tenant_public_ref` EXECUTE on staging            | Durable Auth users/memberships still required |
| P9B5P-02 | Authenticated gateway transport for full read RPC surface              | Prepared       | `rpc-gateway.ts`; `rpc-surface.ts`; adapter tests                                    | App routes still default to synthetic bundle  |
| P9B5P-03 | Remaining read RPC migration + offline contract                        | Implemented    | `20260715220000` applied on staging; verifier expects five versions; apply record    | Production apply still gated                  |
| P9B5P-04 | Explicit demo vs live modes; no silent fallback                        | Prepared       | `repository-config.ts`; `.env.example`; D-026; cutover runbook                       | Preview env still local-demo                  |
